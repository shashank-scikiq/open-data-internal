INSERT INTO POSTGRES_SCHEMA.CATEGORY_TBL (
    order_date,
    category,
    delivery_district,
    delivery_state,
...
SELECT
    ...
    ROUND(100.0 * SUM(intrastate_orders) / NULLIF(SUM(total_orders_delivered), 0), 2) AS intrastate_orders_percentage,
    ROUND(100.0 * SUM(intradistrict_orders) / NULLIF(SUM(total_orders_delivered), 0), 2) AS intradistrict_orders_percentage
FROM
