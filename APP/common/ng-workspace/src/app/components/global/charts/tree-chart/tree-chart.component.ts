...
export class TreeChartComponent implements OnInit, OnChanges {
    ...
    this.mapService.selectedState$.subscribe(
      (state: any) => {
        ...
        if (this.selectedState == 'TT') {
          ...
          this.options = [this.selectedState];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes && !changes['direction']['firstChange']) {
      this.initChart();
    }
  }
  ...
  initChart() {
    ...
    this.isLoading = true;
    this.appService.getTopSellersData(
      this.direction == 'ltr' ? 'seller' : 'delivery',
      this.selectedType, 
      this.selectedStateOption, 
      this.selectedDistrictOption).pipe(takeUntil(this.topSellerSubscription$)).subscribe(
      (response: any) => {
        this.treeData = response;
        this.isLoading = false;
        if (this.direction == 'rtl') { 
          this.create_chart_reverse();
        } else {
          this.create_chart();
        }
      }, (error: Error) => {
  ...
  updateView(option: any) {
    if ((this.selectedState == 'TT') || (this.selectedType == option)) {
      return;
    }
