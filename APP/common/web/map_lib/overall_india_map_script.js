...
async function fetchOrderMetricsSummary(start_date, end_date, category, sub_category, state) {
    try {
        ...
        const url = `${pathname}/api/map_state_data/?${queryParams.toString()}`;
...
async function fetchMapStateData(start_date, end_date, category, sub_category, state) {
    try {
        ...
        const url = `${pathname}/api/map_statewise_data/?${queryParams.toString()}`;
...
async function fetchTopCardsData(startDate, endDate, category, subCategory, domain, state) {
  ...
  top_cards_data = await fetchFromApi(`${pathname}/api/top_card_delta/?${queryParams}`);
}
...
function updateTopCardsDelta(data, prevDateRange) {
    try {
        ...
        updateTopCardMetrics("confirmed-orders-change", data.cnf_delta);
        ...
        updateTopCardMetrics("confirmed-orders-change", 0);
...
async function update_cards(topCardsData, prev_date) {
    try {
        ...
        updateTopCardMetrics("india-activesellers", 0);
