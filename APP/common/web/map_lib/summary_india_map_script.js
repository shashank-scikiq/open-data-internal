...
import {
    loadTopStateOrdersChart, loadTopDistrictOrdersChart,
    loadTopStateLogisticsChart, loadTopDistrictLogisticsChart,
} from './overall-custom-chart.js';
const width = 1200;
const height = 1200;
...
async function fetchOrderMetricsSummary(start_month, start_year, end_month, end_year, category, sub_category, state) {
    try {
//        let pathname = window.location.pathname;
        const baseUrl = `${window.location.origin}${window.location.pathname.replace(/\/$/, '')}`;

        const domainName = 'None'
        const queryParams = new URLSearchParams({ domainName, start_month, start_year, end_month, end_year });
...
async function fetchMapStateData(start_month, start_year, end_month, end_year, category, sub_category, state) {
    try {
        let pathname = window.location.pathname;
        const loader = $('body');
...
async function fetchTopCardsData(start_month, start_year, end_month, end_year, category, subCategory, domain, state) {
  const queryParams = new URLSearchParams({ start_month, start_year, end_month, end_year, category, subCategory, domain, state});
  return fetchFromApi(`top_card_delta/?${queryParams}`);
}

async function loadTopCardsDeltaData(start_month, start_year, end_month, end_year, category, sub_category,domain, state) {
    emptyTopCardMetrics()
    const SessionStatename = localStorage.getItem('statename');
    if (SessionStatename) {
        state = SessionStatename
    }
    fetchTopCardsData(start_month, start_year, end_month, end_year, category, sub_category,domain, state)
...
async function loadMapDataAndVisualizations(start_month, start_year, end_month, end_year, category, sub_category, state) {
    try {
        const [map_state_data, map_statewise_data] = await Promise.all([
            fetchMapStateData(start_month, start_year, end_month, end_year, category, sub_category, state),
            fetchOrderMetricsSummary(start_month, start_year, end_month, end_year, category, sub_category, state),
            loadTopCardsDeltaData(start_month, start_year, end_month, end_year, category, sub_category, 'None', state)
    ...
    buttonChange(statemap_geojson, districtmap_geojson, state_meshdata, maxdata, start_month, start_year, end_month, end_year, category, sub_category, state); // Assuming this doesn't return a Promise
...
export async function main(start_month, start_year, end_month, end_year, category, sub_category, state, filter_flag=1) {

    await loadMapDataAndVisualizations(start_month, start_year, end_month, end_year, category, sub_category, state);
...
function buttonChange(statemap_geojson, districtmap_geojson, state_meshdata, maxdata,
start_month, start_year, end_month, end_year, category, sub_category, state) {
    const btn_india_district_map = d3.select('#india-district-map-btn')
    const btn_india_state_map = d3.select('#india-state-map-btn')
