...
from apps.src.database_utils.dal_retail_overall import RetailDataAccessLayer
...
class RetailViewset(BaseViewSet):
    access_layer = RetailDataAccessLayer()
    ...
    def clean_and_prepare_data(self, merged_df):
        ...
        drop_columns = [
            ...
            'total_sellers_previous', 'delivery_state_code_previous'
        ]
    ...
    def build_state_metrics(self, row):
        metrics = [
            ...
            self.create_metric_data(row['total_sellers_current'], 'Sellers', row.get('total_sellers_count_delta', 0)),
    ...
    def create_metric_data(self, count, heading, delta, count_suffix=''):
        if heading =='Sellers' and count <= 0:
    ...
    def get_map_data(self, request):
        ...
        merged_df = pd.merge(
            ...
            how='outer'
        ).drop(columns=['seller_state_code', 'seller_state', 'seller_district'])
        merged_df = merged_df.fillna(0)
    ...
    @action(detail=False, methods=['get'], url_path='get-max-date')
    @decorator()
    def get_max_date(self, request):
        df = self.access_layer.fetch_date_range()
        fixed_min_date = constant.FIXED_MIN_DATE

        min_year_month = df['min_year_month'].iloc[0]
        max_year_month = df['max_year_month'].iloc[0]

        min_date = f"{min_year_month // 100}-{min_year_month % 100:02d}-01"
        max_date = f"{max_year_month // 100}-{max_year_month % 100:02d}-{calendar.monthrange(max_year_month // 100, max_year_month % 100)[1]:02d}"

        json_result = {
            'min_date': fixed_min_date if fixed_min_date else min_date,
            'max_date': max_date
        }
        return json_result
    
    @action(detail=False, methods=['get'], url_path='state_district_list')
    @decorator()
    def get_state_district_list(self, request):
        district_list_df = self.access_layer.fetch_state_district_list()
