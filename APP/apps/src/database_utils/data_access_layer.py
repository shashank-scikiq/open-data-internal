...
class DataAccessLayer:
    ...
    def fetch_cumulative_orders_statedata_b2b(self, start_date, end_date,
                                              category=None, sub_category=None,
                                              domain=None, state=None):

        table_name = B2B_DISTRICT_TABLE
        query = f'''
            SELECT 
                ...
                ROUND((sum(total_orders_delivered)/SUM(NULLIF(total_items, 0))), 2) AS avg_items_per_order
            FROM {table_name}
            WHERE
    ...
    @log_function_call(ondcLogger)
    def fetch_cumulative_orders_statedata_logistics(self, start_date, end_date,
                                              category=None, sub_category=None,
                                              domain=None, state=None):

        table_name = LOGISTICS_DISTRICT_TABLE
        query = f'''
            SELECT 
                delivery_state_code AS delivery_state_code,
                delivery_state AS delivery_state,
    ...
    @log_function_call(ondcLogger)
    def fetch_cumulative_orders_statewise_b2b(self, start_date, end_date, category=None, sub_category=None, domain=None,
                                              state=None):
        table_name = B2B_DISTRICT_TABLE
        ...
        query = f"""
                SELECT 
                    ...
                    ROUND((SUM(total_items)/SUM(NULLIF(total_orders_delivered,0))),2) AS avg_items_per_order
                FROM {table_name}
                WHERE
                    order_date BETWEEN %s AND %s
            """
    ...
    @log_function_call(ondcLogger)
    def fetch_cumulative_orders_statewise_logistics(self, start_date, end_date, category=None, sub_category=None, domain=None,
                                              state=None):
        table_name = LOGISTICS_DISTRICT_TABLE
    ...
    def fetch_cumulative_orders_statewise(self, start_date, end_date, category=None, sub_category=None, domain=None,
                                          state=None):
        getcontext().prec = 4
        filtered_df = self.fetch_filtered_data(start_date, end_date, category, sub_category, state)

        if domain and 'domain' in filtered_df.columns:
            filtered_df = filtered_df[filtered_df['domain'] == domain]

        filtered_df['total_items'] = filtered_df['total_items'].apply(float)
        filtered_df['total_orders_delivered'] = filtered_df['total_orders_delivered'].apply(float)
        filtered_df['intradistrict_orders'] = filtered_df['intradistrict_orders'].apply(float)
        filtered_df['intrastate_orders'] = filtered_df['intrastate_orders'].apply(float)

        aggregated_df = filtered_df.groupby(['delivery_state_code', 'delivery_state'], as_index=False).agg(
            total_orders=pd.NamedAgg(column='total_orders_delivered', aggfunc='sum'),
            total_items=pd.NamedAgg(column='total_items', aggfunc='sum'),
            intradistrict_orders=pd.NamedAgg(column='intradistrict_orders', aggfunc='sum'),
            intrastate_orders=pd.NamedAgg(column='intrastate_orders', aggfunc='sum'),
        )

        aggregated_df['avg_items_per_order'] = (
                (aggregated_df['total_items'].apply(Decimal)) / aggregated_df['total_orders'].apply(Decimal)).apply(
            float).round(2)
        aggregated_df['intradistrict_percentage'] = ((aggregated_df['intradistrict_orders'].apply(Decimal) /
                                                      aggregated_df['total_orders'].apply(Decimal)) * Decimal(
            100)).apply(float).round(2)
        aggregated_df['intrastate_percentage'] = ((aggregated_df['intrastate_orders'].apply(Decimal) / aggregated_df[
            'total_orders'].apply(Decimal)) * Decimal(100)).apply(float).round(2)

        aggregated_df[['avg_items_per_order', 'intradistrict_percentage', 'intrastate_percentage']] = aggregated_df[
            ['avg_items_per_order', 'intradistrict_percentage', 'intrastate_percentage']].fillna(0)

        aggregated_df['avg_items_per_order'].fillna(0, inplace=True)
        aggregated_df['intradistrict_percentage'].fillna(0, inplace=True)
        aggregated_df['intrastate_percentage'].fillna(0, inplace=True)
        aggregated_df.sort_values(by=['delivery_state', 'total_orders'], ascending=[True, False], inplace=True)

        return aggregated_df

    @log_function_call(ondcLogger)
    def fetch_most_orders_delivered_to_b2b(self, start_date, end_date, category=None,
    ...
    @log_function_call(ondcLogger)
    def fetch_most_orders_delivered_to_logistics(self, start_date, end_date, category=None,
                                           sub_category=None, domain=None, state=None):
        table_name = LOGISTICS_DISTRICT_TABLE
    ...
    @log_function_call(ondcLogger)
    def fetch_total_orders_b2b(self, start_date, end_date, category=None,
                           sub_category=None, domain=None, state=None):
        table_name = B2B_DISTRICT_TABLE
    ...
    def fetch_total_orders_b2b_single(self, start_date, end_date, category=None,
                               sub_category=None, domain=None, state=None):
        table_name = B2B_DISTRICT_TABLE
        ...
        total_orders = 0
        total_items = 0
        if isinstance(orders_count, list) and orders_count:
            try:
                total_orders, total_items = orders_count[0][:2]
            except Exception as e:
                print(f"Error: {e}")
                total_orders = 0
                total_items = 0
    ...
    def fetch_overall_top5_seller_states(self, start_date, end_date, category=None, sub_category=None, domain=None,
        ...
        query += """
                ...
                WHERE sub.rn <= 4
                and upper(sub.seller_state) <> upper('Missing') 
    ...
    def fetch_overall_top5_seller_districts(self, start_date, end_date, category=None, sub_category=None, domain=None,
        ...
        query += """
                ...
                WHERE sub.rn <= 4 
                and upper(sub.delivery_district) <> upper('Missing')
    ...
    @log_function_call(ondcLogger)
    def fetch_total_orders_logistics(self, start_date, end_date, category=None,
                               sub_category=None, domain=None, state=None):
        table_name = LOGISTICS_DISTRICT_TABLE
    ...
    @log_function_call(ondcLogger)
    def fetch_total_sellers_logistics(self, start_date, end_date, category=None,
                                sub_category=None, domain=None, state=None):
        table_name = LOGISTICS_SELLER_TABLE
        ...
        if state:
            query += seller_state_sub_query
            parameters.append(state)

        total_sellers = self.db_utility.execute_query(query, parameters, return_type='sql')
        if isinstance(total_sellers, list) and total_sellers:
            try:
                return total_sellers[0][0]
            except Exception as e:
                print(f"Error: {e}")
                return 0
        else:
    ...
    @log_function_call(ondcLogger)
    def fetch_total_sellers_b2b(self, start_date, end_date, category=None,
                                sub_category=None, domain=None, state=None):
        table_name = B2B_SELLER_TABLE
    ...
    @log_function_call(ondcLogger)
    def fetch_district_count_b2b(self, start_date, end_date, category=None,
                                 sub_category=None, domain=None, state=None):
        table_name = B2B_DISTRICT_TABLE
    ...
    def fetch_cumulative_orders_statewise_summary(self, start_date, end_date,
        ...
        return aggregated_df
    @log_function_call(ondcLogger)
    def fetch_district_count_logistics(self, start_date, end_date, category=None,
                                 sub_category=None, domain=None, state=None):
        table_name = LOGISTICS_DISTRICT_TABLE
    ...
    @log_function_call(ondcLogger)
    def fetch_active_sellers_b2b(self, start_date, end_date, category=None,
                                 sub_category=None, domain=None, state=None):
        table_name = B2B_SELLER_TABLE
    ...
    @log_function_call(ondcLogger)
    def fetch_active_sellers_statedata_b2b(self, start_date, end_date,
                                           category=None, sub_category=None,
                                           domain=None, state=None):
        table_name = B2B_SELLER_TABLE
