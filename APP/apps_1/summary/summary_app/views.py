...
templatePath = 'apps/summary/web/html/'
...
def shift_date_months(start_date_str, end_date_str, months=-1):
    ...
    return new_start_date.strftime("%Y"), new_start_date.strftime("%m"), new_end_date.strftime("%Y"), new_end_date.strftime("%m")
...
class FetchTopCardDeltaData(SummaryBaseDataAPI):
    ...
    def get(self, request, *args, **kwargs):
        """
            SummaryBaseDataAPI FetchTopCardDeltaData
        """
        try:
            params = self.extract_common_params(request)
            original_start_month = params['start_month']
            original_start_year = params['start_year']
            original_end_month = params['end_month']
            original_end_year = params['end_year']
            # previous_start_date, previous_end_date = shift_date_months(original_start_month,
            #                                                            original_start_year,
            #                                                            original_end_month,
            #                                                            original_end_year)
            # format_str = '%Y-%m-%d'
            ...
            total_orders_current = data_service.get_total_orders_summary(**params)
            total_district_count_current = data_service.get_district_count_summary(**params)
            max_orders_delivered_to = data_service.get_most_orders_delivered_to_summary(**params)

            prev_start_year, prev_start_month, prev_end_year, prev_end_month = shift_date_months(str(params['start_year']) + '-' + str(params['start_month']),
                                                     str(params['end_year']) + '-' + str(params['end_month']))

            params['start_month'] = prev_start_month
            params['start_year'] = prev_start_year
            params['end_month'] = prev_end_month
            params['end_year'] = prev_end_year
            total_orders_previous = data_service.get_total_orders_summary(**params)
            total_district_count_previous = data_service.get_district_count_summary(**params)
            format_str = '%Y-%m'
            date_format = '%b, %Y'
            previous_start_date = datetime.strptime(str(params['start_year']) + '-' + str(params['start_month']), format_str)
            previous_end_date = datetime.strptime(str(params['end_year']) + '-' + str(params['end_month']), format_str)
...
class FetchMapStateWiseData(SummaryBaseDataAPI):
    ...
    def get(self, request, *args, **kwargs):
        """
            SummaryBaseDataAPI FetchMapStateWiseData
        """
        try:
            params = self.extract_common_params(request)
...
class FetchMapStateData(SummaryBaseDataAPI):
    ...
    def get(self, request, *args, **kwargs):
        """
            SummaryBaseDataAPI FetchMapStateData
        """
